...
<table>
    ...
    @foreach($checkout as $checkout_element)
    <tr>
        <td>{{$sell_element->id}}</td>
        <td>{{$sell_element->name}}</td>
        <td>{{$sell_element->created_at}}</td>
        <td>
            <div class="desktop-two-columns">
                <div class="column">
                    <div class="panel-button-table edit-button"
                        data-url="{{route('sells_edit', ['product' => $sell_element->id])}}">
                ...
                <div class="column">
                    <div class="panel-button-table delete-button"
                        data-url="{{route('sells_destroy', ['product'=> $sell_element->id])}}">
                        <svg viewBox="0 0 24 24">
                            <path fill="currentColor"
...
<form class="admin-form" action="{{route("sells_store")}}">
    ...
    <div class="material-designs">
        ...
        <div class="escoba" data-url="{{route('users_create')}}">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="24"
                height="24" viewBox="0 0 24 24">
                <path
